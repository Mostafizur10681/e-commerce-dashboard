...
export async function GET(request: Request) {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/contact-settings`, {
      next: { revalidate: 0 }
    });
...
export async function PUT(request: Request) {
  try {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/auth/contact-settings`, {
      method: "PUT",
      headers: {
        "Authorization": token,
