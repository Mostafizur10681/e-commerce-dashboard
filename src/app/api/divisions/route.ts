...
export async function GET(request: Request) {
  try {
    ...
    const backendUrl = new URL((process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000") + "/api/v1/divisions");
...
export async function POST(request: Request) {
  try {
    ...
    const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000") + "/api/admin/divisions", {
      method: "POST",
      headers: {
