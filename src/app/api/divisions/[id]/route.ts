...
export async function GET(
  ...
  try {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/v1/divisions/${id}`, {
      headers: {
...
export async function PUT(
  ...
  try {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/divisions/${id}`, {
      method: "PUT",
      headers: {
