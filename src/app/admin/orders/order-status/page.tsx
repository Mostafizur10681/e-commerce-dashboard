...
export default function OrderStatusPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<OrderStatus | null>(null);
  ...
  const fetchStatuses = async () => {
    try {
      ...
      const url = `/api/order-statuses?q=${encodeURIComponent(searchTerm)}&status=${statusFilter}&page=${currentPage}&limit=${pageSize}`;
      const res = await fetch(url, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (!res.ok) throw new Error("Failed to load order statuses");
      ...
      console.error(err);
      toast(err.message || "An error occurred while loading order statuses", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenEdit = (item: OrderStatus) => {
  ...
  const handleOpenView = (item: OrderStatus) => {
    setSelectedItem(item);
    setIsViewOpen(true);
  };

  const handleOpenDelete = (item: OrderStatus) => {
    setSelectedItem(item);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || "Failed to create order status");
      }
      ...
      toast(err.message || "Failed to create order status", "error");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const token = typeof window !== "undefined" ? localStorage.getItem("df_access_token") : null;
      const res = await fetch(`/api/order-statuses/${selectedItem.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || "Failed to delete order status");
      }

      toast("Order status deleted successfully", "success");
      setIsDeleteOpen(false);
      const isLastItemOnPage = orderStatuses.length === 1;
      ...
      toast(err.message || "Failed to delete order status", "error");
    } finally {
      setSubmitting(false);
    }
  };

  const toggleStatus = async (item: OrderStatus) => {
    try {
      const token = typeof window !== "undefined" ? localStorage.getItem("df_access_token") : null;
      const nextStatus = item.status === "Active" ? "Inactive" : "Active";
      const res = await fetch(`/api/order-statuses/${item.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
      ...
      toast(`Order status marked as ${nextStatus}`, "success");
      fetchStatuses();
    } catch (err: any) {
  ...
  if (!mounted) {
    return (
      <div className="h-96 flex flex-col items-center justify-center gap-3 bg-gray-55 dark:bg-slate-950 transition-colors duration-200">
        ...
        <span className="text-sm font-semibold text-gray-500 dark:text-slate-400">Loading Order Statuses...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 min-h-screen p-6 bg-gray-55 dark:bg-slate-950 text-gray-900 dark:text-slate-100 transition-colors duration-200">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pt-2">
          <div>
            <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
              <ClipboardList className="h-6 w-6 text-green-600 dark:text-green-500" />
              Order Status Management
            </h1>
            <p className="text-sm text-gray-500 dark:text-slate-400">
              Create, edit, and delete order workflow statuses (e.g. Order Placed, Processing, Packed, Shipped, Delivered).
            </p>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="p-16 text-center max-w-sm mx-auto">
            ...
            <h3 className="text-lg font-bold mb-1">No Order Statuses Found</h3>
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-6">
              Create a custom order status or modify search filters.
            </p>
          </div>
        ) : (
          <>
                ...
                >
                  ...
                  <div className="flex items-center justify-end gap-2 pt-1">
                    ...
                    <Button
                      ...
                      size="icon"
                      onClick={() => handleOpenDelete(item)}
                      className="w-9 h-9 rounded-xl hover:bg-red-50 dark:hover:bg-red-955/20 text-red-600 hover:text-red-755 dark:text-red-555 dark:hover:text-red-400"
                      title="Delete"
                    >
                      <Trash2 className="h-4 w-4" />
                    </Button>
        ...
        <DialogContent className="sm:max-w-md rounded-2xl border border-gray-250 dark:border-slate-700 bg-white dark:bg-slate-900">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-500 dark:text-slate-400">
              Create a new status option for orders.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleCreate} className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="e.g. Packed, Shipped, Delivered"
                value={formData.name}
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                placeholder="Description of when this order status applies..."
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="sm:max-w-md rounded-2xl border border-gray-250 dark:border-slate-700 bg-white dark:bg-slate-900">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-500 dark:text-slate-400">
              Modify an existing order status setting.
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <AlertDialogContent className="rounded-2xl border border-gray-250 dark:border-slate-700 bg-white dark:bg-slate-900">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-red-650" />
              Delete Order Status
            </AlertDialogTitle>
            <AlertDialogDescription className="text-gray-550 dark:text-slate-400">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="rounded-xl border-gray-300 dark:border-slate-650">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
