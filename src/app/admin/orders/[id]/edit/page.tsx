...
export default function EditOrderPage() {
  ...
  if (!mounted || loading) {
    return (
      <div className="h-96 flex flex-col items-center justify-center gap-3 bg-gray-50 dark:bg-slate-950 transition-colors duration-200">
  ...
  if (mounted && !order) {
    return (
      <div className="space-y-6 min-h-screen p-6 bg-gray-50 dark:bg-slate-950 flex flex-col items-center justify-center">
        ...
        <Button onClick={() => router.push("/admin/orders")} className="bg-green-600 hover:bg-green-700 text-white rounded-xl mt-4">
          Back to Orders List
        </Button>
      </div>
    );
      ...
      <div className="max-w-xl bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl shadow-sm p-6">
        <form onSubmit={handleSave} className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-gray-700 dark:text-slate-300 block">Fulfillment Status</label>
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value)}
              className="w-full h-11 border border-gray-200 dark:border-slate-800 dark:bg-slate-950 rounded-xl px-3 py-1 text-sm outline-none focus:ring-1 focus:ring-green-500 text-gray-700 dark:text-slate-400 cursor-pointer"
            >
