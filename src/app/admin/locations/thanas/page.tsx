...
import { Thana, District, Division } from "@/types";
...
export default function ThanasPage() {
  ...
  const [sortBy, setSortBy] = useState("thana_name");
  ...
  useEffect(() => {
    const fetchDivisionsList = async () => {
      try {
        ...
        console.error("Failed to load divisions for filter", err);
      }
    };
    fetchDivisionsList();
  }, []);

  // Fetch districts when division filter changes (cascading filter dropdown)
  useEffect(() => {
    const fetchDistrictsList = async () => {
      try {
        const token = typeof window !== "undefined" ? localStorage.getItem("df_access_token") : null;
        let url = "/api/districts?limit=200";
        ...
        console.error("Failed to load districts list for filter", err);
      }
    };
    fetchDistrictsList();
    // Reset district filter if division changes
  ...
  const fetchThanas = async () => {
    try {
      ...
      const token = typeof window !== "undefined" ? localStorage.getItem("df_access_token") : null;
      let url = `/api/thanas?q=${encodeURIComponent(searchTerm)}&status=${statusFilter}&page=${currentPage}&limit=${pageSize}&sort_by=${sortBy}&sort_order=${sortOrder}`;
      ...
      if (!res.ok) throw new Error("Failed to load thanas");
      const json = await res.json();
      
      if (json.success && json.data) {
        setThanas(json.data.data || []);
        setTotal(json.data.total || 0);
        setTotalPages(json.data.last_page || 1);
      } else {
        throw new Error(json.message || "Failed to load thanas");
      }
    } catch (err: any) {
      console.error(err);
      setError(err.message || "An error occurred while loading thanas");
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [searchTerm, statusFilter, divisionFilter, districtFilter, currentPage, sortBy, sortOrder]);
  ...
  return (
    <div className="space-y-6 min-h-screen p-6 bg-gray-55 dark:bg-gray-955 transition-colors duration-300">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1.5">
          ...
          <p className="text-sm text-gray-555 dark:text-gray-400">
            Manage administrative thanas/upazilas of Bangladesh.
          </p>
        </div>
        <Button
          onClick={() => router.push("/admin/locations/thanas/add")}
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between bg-white dark:bg-slate-900 p-4 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-xs transition-colors">
        {/* Search */}
        <div className="relative flex-1 max-w-md">
          <Search className="absolute top-2.5 left-3.5 h-4 w-4 text-gray-400" />
          <Input
            placeholder="Search thanas by name or code..."
            value={searchTerm}
            onChange={(e) => {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-xs overflow-hidden transition-colors">
        {error ? (
          <div className="p-12 text-center">
            ...
            <h3 className="text-base font-semibold mb-1 text-gray-905 dark:text-white">Could not load thanas</h3>
            <p className="text-sm text-gray-505 dark:text-gray-400 mb-4">{error}</p>
            <Button onClick={fetchThanas} variant="outline" className="rounded-xl border-gray-200 dark:border-slate-850 hover:bg-gray-50 dark:hover:bg-slate-800 cursor-pointer">
              Try Again
            </Button>
          </div>
        ) : loading && thanas.length === 0 ? (
          /* Skeletal Table Load */
          ...
          <div className="p-16 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">No thanas found</h3>
            <p className="text-sm text-gray-505 dark:text-gray-400 max-w-sm mx-auto mb-6">
              {searchTerm || statusFilter !== "All" || divisionFilter !== "All" || districtFilter !== "All"
                ? "No thanas matches your search criteria or filters."
                : "Create thanas to start configuring your platform locations."}
            </p>
            {(searchTerm || statusFilter !== "All" || divisionFilter !== "All" || districtFilter !== "All") && (
              <Button
                variant="outline"
        ...
        {!error && thanas.length > 0 && (
          <div className="px-6 py-4 flex items-center justify-between border-t border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 transition-colors">
            <div className="text-xs text-gray-505 dark:text-gray-450 font-medium">
              Showing <span className="font-semibold text-gray-900 dark:text-white">{thanas.length}</span> of{" "}
