    ...
    const fetchCategories = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setCategories(list);
          if (list.length > 0) {
            const activeList = list.filter((c: any) => c.status === true || c.status === "active" || c.status === 1 || c.status === "Active");
            if (activeList.length > 0) {
              setCategory(activeList[0].name);
            } else {
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-6 rounded-2xl shadow-sm space-y-5">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full h-10 border border-gray-200 dark:border-gray-850 dark:bg-gray-955 rounded-xl px-3 py-1 text-sm outline-none focus:ring-1 focus:ring-[#16A34A] text-gray-750 dark:text-gray-300 cursor-pointer disabled:opacity-50"
              >
                {categoriesLoading ? (
